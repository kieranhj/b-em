...
uint8_t readmem(uint16_t addr)
{
        ...
        switch (addr&~3)
        {
                ...
                if (!MASTER)
                {
                        if (WD1770) return wd1770_read(addr);
...
void writemem(uint16_t addr, uint8_t val)
{
        ...
        if (debugon) debug_write(addr,val);
        writec[addr] = 31;
        ...
        switch (addr&~3)
        {
                ...
                if (!MASTER)
                {
                        if (WD1770) wd1770_write(addr, val);
